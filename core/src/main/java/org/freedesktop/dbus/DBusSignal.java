...
public class DBusSignal extends Message {
    ...
    private static final Logger log = Logger.getLogger(DBusSignal.class);
    ...
    public DBusSignal ( String source, String path, String iface, String member, String sig, Object... args ) throws DBusException {
        ...
        long cnt = this.bytecounter;
        if ( null != sig )
            append(sig, args);
        marshallint(this.bytecounter - cnt, this.blen, 0, 4);
        this.bodydone = true;
    }
    ...
    static DBusSignal createSignal ( Class<? extends DBusSignal> c, String source, String objectpath, String sig, long serial, Object... parameters )
            throws DBusException {
        String type = "";
    ...
    DBusSignal createReal ( AbstractConnection conn ) throws DBusException {
        ...
        if ( null == signame )
            signame = getName();
        if ( null == this.c )
            this.c = createSignalClass(conn, intname, signame);

        if ( log.isDebugEnabled() ) {
            log.debug("Converting signal to type: " + this.c);
        }
        Type[] types = typeCache.get(this.c);
        Constructor<? extends DBusSignal> con = conCache.get(this.c);
        if ( null == types ) {
            con = (Constructor<? extends DBusSignal>) this.c.getDeclaredConstructors()[ 0 ];
        ...
        try {
            ...
            if ( null == args )
                ...
                if ( log.isDebugEnabled() ) {
                    log.debug("Creating signal of type " + this.c + " with parameters " + Arrays.deepToString(params));
                }
    ...
    protected DBusSignal ( String objectpath, Object... args ) throws DBusException {
        ...
        this.headers.put(Message.HeaderField.PATH, objectpath);
        this.headers.put(Message.HeaderField.MEMBER, member);
        this.headers.put(Message.HeaderField.INTERFACE, iface);
