...
public class Message {
        ...
        if ( this.preallocated > 0 ) {
            if ( this.paofs + buf.length > this.pabuf.length )
                throw new ArrayIndexOutOfBoundsException(String.format(
                    "Array index out of bounds, paofs=%s, pabuf.length=%s, buf.length=%s.",
                    this.paofs,
                    this.pabuf.length,
                    buf.length));
    ...
    protected void appendByte ( byte b ) {
        if ( this.preallocated > 0 ) {
            ...
            this.wiredata[ this.bufferuse++ ] = new byte[] {
                b
            };
            this.bytecounter++;
        }
    ...
    private Object extractone ( byte[] sigb, byte[] buf, int[] ofs, boolean contained ) throws DBusException {
        ...
        case ArgumentType.DICT_ENTRY1:
            ...
            if ( log.isTraceEnabled() ) {
                Hex h = new Hex();
                log.trace("Extracting Dict Entry (" + h.encode(Arrays.copyOfRange(sigb, ofs[ 0 ], sigb.length - ofs[ 0 ])) + ") from: "
                        + h.encode(Arrays.copyOfRange(buf, ofs[ 1 ], buf.length - ofs[ 1 ])));
            }
            ofs[ 0 ]++;
    ...
    public void setSource ( String source ) throws DBusException {
        if ( null != this.body ) {
            ...
            this.bytecounter = 0;
            preallocate(12);
            append("yyyyuu", this.big ? Endian.BIG : Endian.LITTLE, this.type, this.flags, this.protover, this.bodylen, this.serial);
