...
public class Profile {
    ...
    static class ProfileHandler implements DBusSigHandler<Profiler.ProfileSignal> {
        ...
        public void handle ( Profiler.ProfileSignal s ) {
            if ( 0 == ( this.c++ % Profile.SIGNAL_INNER ) )
                System.out.print("-");
        }
    }
    ...
    public static class Log {
        ...
        public double stddev () {
            ...
            double sum = 0;
            for ( int i = 0; i < this.current; i++ )
                sum += ( this.deltas[ i ] - mean ) * ( this.deltas[ i ] - mean );
            return Math.sqrt(sum / ( this.current - 1 ));
        }
    }
        ...
        try ( DBusConnection conn = DBusConnection.getConnection(DBusConnection.SESSION) ) {
            ...
            if ( "pings".equals(args[ 0 ]) ) {
                ...
                while ( len <= 32768 ) {
                    ...
                    System.out.println(len + " byte array) " + ( count * len ) + " bytes in " + ms + "ms (in " + count + " calls / " + (int) cps
                            + " CPS): " + rate + "MB/s");
                    len <<= 1;
                }
                len = 256;
                while ( len <= 32768 ) {
                    ...
                    double cps = ( count * 1000 ) / ms;
                    double rate = ( len * cps ) / ( 1024.0 * 1024.0 );
                    System.out.println(len + " string) " + ( count * len ) + " bytes in " + ms + "ms (in " + count + " calls / " + (int) cps
                            + " CPS): " + rate + "MB/s");
                    len <<= 1;
                }
            }
